...
public class NexmarkMaxAuctionCount extends splitBolt {
    private static final Logger LOG = LoggerFactory.getLogger(NexmarkMaxAuctionCount.class);
    ...
    private long maxAuctionCount = -1;

    public NexmarkMaxAuctionCount () {
    ...
    public void execute(TransferTuple in) throws InterruptedException {
        ...
        for (int i = 0; i < bound; i ++) {
            NexmarkOutputTuple tuple = (NexmarkOutputTuple) in.getValue(0, i);
            // long key = tuple.auction;
            // long v = counts.getOrDefault(key, 0L) + 1;

            // counts.put(key, v);
            if (maxAuctionCount < tuple.count) {
                maxAuctionCount = tuple.count;
            }
